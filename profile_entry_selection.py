...
def profile_entry_selection():
    ...
    event_handler = EventHandler(
        table, entry_editor, dummy_get_current_po, dummy_update_table, dummy_show_status
    )
    ...
    for row in range(total_rows):
        ...
        print(
            f"Row {row} 初回選択時間: {(row_end_time - row_start_time) * 1000:.2f} ms"
        )
