...
class TestEntryReviewData(unittest.TestCase):
    ...
    def test_review_comments(self):
        ...
        # レビューコメントを追加
        comment_id = self.entry.add_review_comment(author="テスト太郎", comment="これはテストコメントです。")
        
        # コメントが追加されたことを確認
        self.assertEqual(len(self.entry.review_comments), 1)
        self.assertEqual(self.entry.review_comments[0]["author"], "テスト太郎")
        self.assertEqual(self.entry.review_comments[0]["comment"], "これはテストコメントです。")
        self.assertEqual(self.entry.review_comments[0]["id"], comment_id)
        
        # さらにコメントを追加
        comment_id2 = self.entry.add_review_comment(author="テスト花子", comment="二つ目のコメントです。")
