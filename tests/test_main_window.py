...
def mock_components(monkeypatch):
    ...
    # 各モジュールのモックを設定
    monkeypatch.setattr('sgpo_editor.gui.widgets.entry_editor.EntryEditor',
                        MagicMock(return_value=mock_entry_editor))
    monkeypatch.setattr('sgpo_editor.gui.widgets.stats_widget.StatsWidget',
                        MagicMock(return_value=mock_stats_widget))
    monkeypatch.setattr('sgpo_editor.gui.widgets.search_widget.SearchWidget',
                        MagicMock(return_value=mock_search_widget))
    monkeypatch.setattr('sgpo_editor.gui.table_manager.TableManager',
                        MagicMock(return_value=mock_table_manager))
    monkeypatch.setattr('sgpo_editor.gui.file_handler.FileHandler',
                        MagicMock(return_value=mock_file_handler))
    monkeypatch.setattr('sgpo_editor.gui.event_handler.EventHandler',
                        MagicMock(return_value=mock_event_handler))
    monkeypatch.setattr('sgpo_editor.gui.ui_manager.UIManager',
                        MagicMock(return_value=mock_ui_manager))
...
class TestMainWindow:
    ...
    def test_file_operations(self, mock_main_window, monkeypatch) -> None:
        """ファイル操作のテスト"""
        from tests.mock_helpers import mock_message_box_information, mock_message_box_question
        ...
        mock_message_box_question(monkeypatch, return_value=QtWidgets.QMessageBox.StandardButton.Yes)
    ...
    def test_file_operations_error(self, mock_main_window, monkeypatch) -> None:
        ...
        from tests.mock_helpers import mock_file_dialog_get_save_file_name
        ...
        mock_status_bar.showMessage.assert_called_with("エラー: ファイルパスが指定されていません", 3000)
    ...
    def test_layout_with_entry(self, mock_main_window) -> None:
        ...
        assert mock_entry.msgctxt == "test_context", "EntryModelのmsgctxtが正しく設定されていません"

        # エントリを設定
        mock_main_window.entry_editor.set_entry(mock_entry)
        mock_main_window.entry_editor.context_edit.text = MagicMock(return_value="test_context")
        mock_main_window.entry_editor.msgid_edit.toPlainText = MagicMock(return_value="test_msgid")
        mock_main_window.entry_editor.msgstr_edit.toPlainText = MagicMock(return_value="test_msgstr")
        mock_main_window.entry_editor.get_layout_type = MagicMock(return_value=LayoutType.LAYOUT1)

        # 初期状態でコンテキストが正しく設定されていることを確認
        initial_context = mock_main_window.entry_editor.context_edit.text()
        assert initial_context == "test_context", "初期状態でコンテキストが正しく設定されていません"
        ...
        # レイアウト2に切り替え
        mock_main_window.entry_editor.get_layout_type = MagicMock(return_value=LayoutType.LAYOUT2)
        mock_layout2_action.trigger()

        # レイアウト2に切り替わったことを確認
        assert mock_main_window.entry_editor.get_layout_type() == LayoutType.LAYOUT2, "レイアウト2への切り替えが失敗しました"

        # レイアウト2でのコンテキストを確認
        # モックの設定を更新して異なるレイアウトでもコンテキストが保持されていることをシミュレート
        layout2_context = mock_main_window.entry_editor.context_edit.text()
        assert layout2_context == "test_context", f"レイアウト2でコンテキストが失われました: expected='test_context', actual='{layout2_context}'"

        # エントリの内容が保持されていることを確認
        assert mock_main_window.entry_editor.msgid_edit.toPlainText() == "test_msgid", "レイアウト2でmsgidが失われました"
        assert mock_main_window.entry_editor.msgstr_edit.toPlainText() == "test_msgstr", "レイアウト2でmsgstrが失われました"
        ...
        # レイアウト1に切り替わったことを確認
        assert mock_main_window.entry_editor.get_layout_type() == LayoutType.LAYOUT1, "レイアウト1への切り替えが失敗しました"

        # レイアウト1でのコンテキストを確認
        layout1_context = mock_main_window.entry_editor.context_edit.text()
        assert layout1_context == "test_context", f"レイアウト1でコンテキストが失われました: expected='test_context', actual='{layout1_context}'"

        # エントリの内容が保持されていることを確認
        assert mock_main_window.entry_editor.msgid_edit.toPlainText() == "test_msgid", "レイアウト1でmsgidが失われました"
        assert mock_main_window.entry_editor.msgstr_edit.toPlainText() == "test_msgstr", "レイアウト1でmsgstrが失われました"
    ...
    def test_entry_list_layout(self, mock_main_window):
        ...
        mock_main_window.table.horizontalHeaderItem = MagicMock(side_effect=lambda col: header_items[col])

        # 列数が5であることを確認
        assert mock_main_window.table.columnCount() == 5, "エントリリストは5列でなければなりません"
    ...
    def test_table_sorting(self, mock_main_window) -> None:
        ...
        # table.itemメソッドをモック
        mock_main_window.table.item = lambda row, col: mock_items.get((row, col), MagicMock())
        ...
        # モックの値を更新
        mock_main_window.table_manager._current_sort_order = Qt.SortOrder.DescendingOrder
    ...
    def test_state_based_filtering(self, mock_main_window) -> None:
        ...
        for state, filtered_entries in entries.items():
            ...
            # 行数を検証
            assert mock_main_window.table.rowCount() == expected_count, f"フィルタ '{state}' での件数が期待通りでない"
            ...
            for i, entry in enumerate(filtered_entries):
                ...
                assert msgid_item.text() == entry.msgid, f"{i}行目のmsgidが期待値と異なります"
    ...
    def test_gui_state_filter_interaction(self, mock_main_window) -> None:
        ...
        def fake_get_filtered_entries(filter_text: str, search_text: str, sort_column: str | None,
                                      sort_order: str | None):
            if filter_text == "翻訳済み":
        ...
        from sgpo_editor.gui.widgets.search import SearchCriteria
        mock_main_window.search_widget.get_search_criteria = lambda: SearchCriteria(filter="翻訳済み", search_text="",
                                                                                    match_mode="部分一致")
        ...
        # 行数を検証
        assert mock_main_window.table.rowCount() == 1, "GUI上の状態フィルタが正しく機能していません"
    ...
    def test_gui_keyword_filter_interaction(self, mock_main_window) -> None:
        ...
        # 検索条件を予め定義
        search_criteria = SearchCriteria(filter="すべて", search_text="app", match_mode="部分一致")
        ...
        # 行数を検証
        assert mock_main_window.table.rowCount() == expected_count, "GUI上の部分一致キーワードフィルタが正しく機能していません"
    ...
    def test_view_menu_layout(self, mock_main_window) -> None:
        ...
        mock_entry_edit_menu.actions.return_value = [mock_layout1_action, mock_layout2_action]
        ...
        assert layout1_action.text() == "レイアウト1", "レイアウト1のテキストが正しくありません"
        assert layout2_action.text() == "レイアウト2", "レイアウト2のテキストが正しくありません"
        assert layout1_action.isCheckable(), "レイアウト1アクションがチェック可能ではありません"
        assert layout2_action.isCheckable(), "レイアウト2アクションがチェック可能ではありません"
