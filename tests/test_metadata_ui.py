...
from PySide6.QtWidgets import QApplication, QPushButton, QTableWidgetItem, QMessageBox
...
class TestMetadataEditDialog:
    ...
    def test_init_and_load_metadata(self, app, entry, qtbot):
        ...
        assert json.loads(dialog.metadata_table.item(2, 1).text()) == ["important", "needs-review"]
        
        assert dialog.metadata_table.item(3, 0).text() == "context"
        assert json.loads(dialog.metadata_table.item(3, 1).text()) == {"screen": "login", "position": "header"}
