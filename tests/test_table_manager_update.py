...
class TestTableManagerUpdate(unittest.TestCase):
    ...
    def test_update_table_with_hidden_columns(self):
        ...
        # テーブル更新後も非表示状態が維持されているか確認
        self.assertIn(hidden_column, self.table_manager._hidden_columns)
        self.table.setColumnHidden.assert_any_call(hidden_column, True)
    ...
    def test_toggle_column_after_update(self):
        ...
        # _apply_column_visibilityメソッドをモック
        with patch.object(self.table_manager, '_apply_column_visibility', new=lambda: None):
            # テーブル更新処理を実行
            with patch('sgpo_editor.gui.table_manager.QTableWidgetItem'):
                self.table_manager._update_table_contents(entries)
            ...
            # テーブルのsetColumnHiddenが呼ばれたか確認
            self.table.setColumnHidden.assert_any_call(toggle_column, True)
            ...
            self.table.setColumnHidden.assert_any_call(toggle_column, False)
