...
def sample_entries():
    ...
    entries[1].add_metadata(
        METADATA_EVALUATION_METRICS, {"accuracy": 90, "fluency": 80}
    )
    ...
    entries[1].add_metadata(
        ...
        [
            ...
            {
                "author": "Test User",
                "comment": "The translation quality is good",
                "language": "en",
            },
        ],
    )
...
class TestTranslationEvaluateDialog:
    ...
    @patch(
        "sgpo_editor.gui.translation_evaluate_dialog.TranslationEvaluationResultWindow"
    )
    def test_run_evaluation_with_entry(
        self, mock_result_window, mock_info, dialog, sample_entries
    ):
        """エントリありでの評価実行テスト"""
        # エントリを設定
    ...
    @patch("sgpo_editor.gui.translation_evaluate_dialog.QMessageBox.information")
    def test_run_evaluation_unevaluated_entries(
        self, mock_info, dialog, sample_entries
    ):
        """未評価エントリのみの評価テスト"""
        # エントリを設定
        dialog.set_entries(sample_entries)
...
class TestTranslationEvaluationResultWindow:
    ...
    def test_always_on_top_toggle(self, result_window):
        ...
        try:
            ...
            assert (
                result_window._toggle_always_on_top.call_args[0][0] == 2
            )  # Qt.CheckState.Checked の値は2
            ...
            result_window._toggle_always_on_top.assert_called_once()
            assert (
                result_window._toggle_always_on_top.call_args[0][0] == 0
            )  # Qt.CheckState.Unchecked の値は0
        finally:
            # 元のメソッドを復元
    ...
    def test_comment_language_switch(self, result_window):
        ...
        assert (
            ...
            in result_window.comment_text.toPlainText()
        )
