...
class TestColumnVisibilityGUI:
    ...
    def test_initial_column_visibility(self, main_window):
        ...
        for i in range(table.columnCount()):
            assert not table.isColumnHidden(i), (
                f"列 {i} が初期状態で非表示になっています"
            )
            assert table_manager.is_column_visible(i), (
                f"列 {i} のis_column_visible()がFalseを返しました"
            )
    ...
    def test_toggle_column_visibility(self, main_window):
        ...
        # 列が非表示になっているか確認
        assert table.isColumnHidden(column_index), (
            f"列 {column_index} ({column_name}) が非表示になっていません"
        )
        assert not table_manager.is_column_visible(column_index), (
            f"列 {column_index} のis_column_visible()がTrueを返しました"
        )
        assert not main_window.ui_manager.column_visibility_actions[
            column_index
        ].isChecked(), (
        ...
        # 列が表示されているか確認
        assert not table.isColumnHidden(column_index), (
            f"列 {column_index} ({column_name}) が再表示されていません"
        )
        assert table_manager.is_column_visible(column_index), (
            f"列 {column_index} のis_column_visible()がFalseを返しました"
        )
        assert main_window.ui_manager.column_visibility_actions[
            column_index
        ].isChecked(), f"列 {column_index} のメニュー項目が正しくチェックされていません"
    ...
    def test_menu_action_triggers_toggle(self, main_window):
        ...
        column_name = table_manager.get_column_name(column_index)
        print(
            f"テスト: メニューアクションで列 {column_index} ({column_name}) の表示状態を切り替えます"
        )
    ...
    def test_settings_persistence(self, main_window, app):
        ...
        for i in range(new_table.columnCount()):
            if i in visible_columns:
                assert not new_table.isColumnHidden(i), (
                    f"新ウィンドウで列 {i} が表示されていません"
                )
            else:
                assert new_table.isColumnHidden(i), (
                    f"新ウィンドウで列 {i} が非表示になっていません"
                )
