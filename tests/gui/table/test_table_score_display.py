...
def test_table_score_column_display(table_widget, entry_data):
    ...
    # スコアの表示内容を確認
    assert table_widget.item(0, 5).text() == "-", (
        "スコアなしのエントリは'-'と表示されるべき"
    )
    assert table_widget.item(1, 5).text() == "90", (
        "高スコアのエントリは数値が表示されるべき"
    )
    assert table_widget.item(2, 5).text() == "70", (
        "中スコアのエントリは数値が表示されるべき"
    )
    assert table_widget.item(3, 5).text() == "40", (
        "低スコアのエントリは数値が表示されるべき"
    )
    ...
    assert table_widget.item(1, 5).background().color().rgb() == light_green.rgb(), (
        "高スコアのエントリは薄緑で表示されるべき"
    )
    assert table_widget.item(2, 5).background().color().rgb() == light_yellow.rgb(), (
        "中スコアのエントリは薄黄で表示されるべき"
    )
    assert table_widget.item(3, 5).background().color().rgb() == light_red.rgb(), (
        "低スコアのエントリは薄赤で表示されるべき"
    )
...
def test_table_score_sorting(table_widget, entry_data):
    ...
    sorted_entries_asc = table_manager._sort_entries_by_score(
        entry_data, Qt.SortOrder.AscendingOrder
    )

    # スコア値を直接取得して確認
    scores_asc = [
        entry.overall_quality_score()
        if hasattr(entry, "overall_quality_score")
        else None
    ...
    sorted_entries_desc = table_manager._sort_entries_by_score(
        entry_data, Qt.SortOrder.DescendingOrder
    )
...
def test_table_score_column_visibility(table_widget):
    ...
    # 初期状態では列は表示されているはず
    assert not table_widget.isColumnHidden(5), (
        "初期状態ではスコア列は表示されているべき"
    )
