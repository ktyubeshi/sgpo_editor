...
class ReviewDialogFacade(QObject):
    ...
    def register_dialog(self, dialog_type: str, dialog: QWidget) -> None:
        ...
        """
        logger.debug(
            f"ReviewDialogFacade.register_dialog: ダイアログ {dialog_type} を登録"
        )
    ...
    def update_entry_field(
        self, entry: EntryModel, field_name: str, value: Any
    ) -> bool:
        """エントリのフィールドを更新
        ...
        if not self._database or not entry:
            logger.debug(
                "ReviewDialogFacade.update_entry_field: データベースまたはエントリがない"
            )
            return False

        try:
            logger.debug(
                f"ReviewDialogFacade.update_entry_field: フィールド {field_name} を更新"
            )
            self._database.update_entry_field(entry.key, field_name, value)
            return True
        except Exception as e:
            logger.error(
                f"ReviewDialogFacade.update_entry_field: エラー発生 {e}", exc_info=True
            )
            return False
...
class EntryEditorFacade(QObject):
    ...
    def set_database(self, db) -> None:
        ...
        logger.debug("EntryEditorFacade.set_database: 完了")
