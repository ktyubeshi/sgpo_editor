...
class TranslationEvaluateDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self._database: Optional[InMemoryEntryStore] = None
    ...
    def set_database(self, database: InMemoryEntryStore) -> None:
        """データベースを設定
        ...
        logger.debug("InMemoryEntryStore set for translation evaluation dialog")
    ...
    def _run_evaluation(self) -> None:
        ...
        if target_mode == 0:  # 現在選択中のエントリのみ
            ...
            target_entries = [
                ...
                if e.evaluation_state == EvaluationState.NOT_EVALUATED
            ]
        ...
        for i, entry in enumerate(target_entries):
            ...
            # メタデータとして評価結果を保存
            self._save_evaluation_to_metadata(
                entry, overall_score, metric_scores, model, language
            )
    ...
    def _save_evaluation_to_metadata(
        self,
        entry: EntryModel,
        score: int,
        metric_scores: LLMResponseMetricScores,
...
class TranslationEvaluationResultWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        self.always_on_top_check.setToolTip(
            "チェックすると、このウィンドウが常に他のウィンドウの前面に表示されます"
        )
        ...
        self.comment_language_combo.currentIndexChanged.connect(
    ...
    def set_entry(self, entry: Optional[EntryModel]) -> None:
        ...
        try:
            if entry:
                logger.debug(
                    f"TranslationEvaluationResultWindow.set_entry: エントリを設定 msgid='{entry.msgid[:30]}...'"
                )
                # エントリのキーとメタデータを確認
                entry_key = getattr(entry, "key", None)
                metadata = (
                    entry.get_all_metadata()
                    if hasattr(entry, "get_all_metadata")
                    else {}
                )
                logger.debug(
                    f"TranslationEvaluationResultWindow.set_entry: エントリキー={entry_key}, メタデータキー数={len(metadata)}"
                )

                # 評価関連のメタデータを確認
                score = metadata.get(
                    METADATA_EVALUATION_SCORE, getattr(entry, "score", None)
                )
                state = metadata.get(
                    METADATA_EVALUATION_STATE, getattr(entry, "evaluation_state", None)
                )
                logger.debug(
                    f"TranslationEvaluationResultWindow.set_entry: スコア={score}, 状態={state}"
                )
            else:
                logger.debug(
                    "TranslationEvaluationResultWindow.set_entry: Noneエントリを設定"
                )
            ...
            logger.error(
                f"TranslationEvaluationResultWindow.set_entry: エラー発生 {e}",
                exc_info=True,
            )
    ...
    def _update_ui(self) -> None:
        ...
        try:
            if not self._entry:
                logger.debug(
                    "TranslationEvaluationResultWindow._update_ui: エントリがないため表示をクリア"
                )
                # エントリがない場合は表示をクリア
                ...
                logger.debug(
                    "TranslationEvaluationResultWindow._update_ui: 表示クリア完了"
                )
                return

            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: エントリ情報を表示 msgid='{self._entry.msgid[:30]}...'"
            )
            ...
            logger.debug(
                "TranslationEvaluationResultWindow._update_ui: 翻訳テキスト設定完了"
            )

            # メタデータから評価情報を取得
            metadata = self._entry.get_all_metadata()
            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: メタデータキー数={len(metadata)}"
            )

            # 評価状態の更新
            evaluation_state = metadata.get(
                METADATA_EVALUATION_STATE, str(self._entry.evaluation_state)
            )
            self.state_value.setText(evaluation_state)
            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: 評価状態={evaluation_state}"
            )
            ...
            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: スコア={score_text}"
            )
            ...
            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: 評価モデル={model}"
            )
            ...
            if date != "--":
                ...
                try:
                    ...
                    logger.error(
                        f"TranslationEvaluationResultWindow._update_ui: 日時変換エラー {e}"
                    )
            self.date_value.setText(date)
            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: 評価日時={date}"
            )
            ...
            if not metric_scores:
                ...
                logger.debug(
                    "TranslationEvaluationResultWindow._update_ui: メタデータにスコアがないためエントリから直接取得"
                )

            logger.debug(
                f"TranslationEvaluationResultWindow._update_ui: 指標別スコア数={len(metric_scores)}"
            )
            ...
            for key, display_name in metric_mapping.items():
                if display_name in self.metric_scores:
                    ...
                    logger.debug(
                        f"TranslationEvaluationResultWindow._update_ui: 指標 {display_name}={value_text}"
                    )

            # コメント表示の更新
            logger.debug(
                "TranslationEvaluationResultWindow._update_ui: コメント表示更新開始"
            )
            self._update_comment_display()
            logger.debug("TranslationEvaluationResultWindow._update_ui: UI更新完了")
        except Exception as e:
            logger.error(
                f"TranslationEvaluationResultWindow._update_ui: エラー発生 {e}",
                exc_info=True,
            )
    ...
    def _update_comment_display(self) -> None:
        ...
        if not self._entry:
            logger.debug(
                "TranslationEvaluationResultWindow._update_comment_display: エントリがないためコメント表示をクリア"
            )
            self.comment_text.setText("エントリが選択されていません")
            return
        ...
        logger.debug(
            f"TranslationEvaluationResultWindow._update_comment_display: 選択言語={language}"
        )
        ...
        if language == "日本語":
            ...
            comment = next(
                (
                    c["comment"]
                    for c in comments
                    if "ja" in c.get("language", "").lower()
                    or "japanese" in c.get("language", "").lower()
                ),
                None,
            )
            if comment:
                logger.debug(
                    "TranslationEvaluationResultWindow._update_comment_display: 日本語コメントを見つけました"
                )
        else:
            # 英語のコメントを探す
            comment = next(
                (
                    c["comment"]
                    for c in comments
                    if "en" in c.get("language", "").lower()
                    or "english" in c.get("language", "").lower()
                ),
                None,
            )
            if comment:
                logger.debug(
                    "TranslationEvaluationResultWindow._update_comment_display: 英語コメントを見つけました"
                )
        ...
        logger.debug(
            "TranslationEvaluationResultWindow._update_comment_display: コメント表示更新完了"
        )

    def closeEvent(self, event):
        """ウィンドウが閉じられるときのイベント処理"""
        logger.debug(
            "TranslationEvaluationResultWindow.closeEvent: ウィンドウが閉じられます"
        )
        ...
        if parent and hasattr(parent, "_evaluation_result_window"):
            logger.debug(
                "TranslationEvaluationResultWindow.closeEvent: 親ウィンドウの参照を削除"
            )
    ...
    def _toggle_always_on_top(self, state: int) -> None:
        ...
        if is_checked != is_on_top:
            ...
            if self.isVisible():
                ...
                logger.debug(
                    f"翻訳評価結果ウィンドウの最前面表示状態: {is_on_top_after}"
                )
            else:
                # 非表示の場合はフラグのみ設定
                self.setWindowFlags(new_flags)
                logger.debug(
                    f"翻訳評価結果ウィンドウのフラグを設定（非表示状態）: 最前面={is_checked}"
                )
