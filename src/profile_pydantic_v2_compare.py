...
try:
    ...
    from sgpo_editor.types import EntryDict, EntryDictList, FlagConditions, FilterSettings
...
def create_mock_entry_dicts(num_entries: int) -> EntryDictList:
    ...
    for i in range(num_entries):
        ...
        if fuzzy: fuzzy_count += 1
        if not msgstr and not fuzzy : untranslated_count += 1
        if msgstr and not fuzzy: translated_count += 1
...
def test_filtering(env: Dict[str, Any]):
    ...
    filter_kw_status_set: Set[str] = {TranslationStatus.TRANSLATED, TranslationStatus.FUZZY}
    ...
    time_validate_kw = time_operation("キーワードあり", filter_with_validate, KEYWORD_TO_SEARCH, filter_kw_status_set)
    time_validate_no_kw = time_operation("キーワードなし", filter_with_validate, None, filter_kw_status_set)
    ...
    time_construct_kw = time_operation("キーワードあり", filter_with_construct, KEYWORD_TO_SEARCH, filter_kw_status_set)
    time_construct_no_kw = time_operation("キーワードなし", filter_with_construct, None, filter_kw_status_set)

    # --- 結果サマリ ---
    logger.info("\n--- フィルタリング 結果サマリ ---")
    logger.info(f"キーワードあり: model_validate = {time_validate_kw:.6f} 秒, model_construct = {time_construct_kw:.6f} 秒")
    logger.info(f"キーワードなし: model_validate = {time_validate_no_kw:.6f} 秒, model_construct = {time_construct_no_kw:.6f} 秒")
...
def test_get_single_entry(env: Dict[str, Any]):
    ...
    # --- 結果サマリ ---
    logger.info("\n--- 単一取得 結果サマリ ---")
    logger.info(f"平均取得時間: model_validate = {validate_avg_time * 1000:.6f} ミリ秒, model_construct = {construct_avg_time * 1000:.6f} ミリ秒")
...
def test_get_multiple_entries(env: Dict[str, Any]):
    ...
    time_validate = time_operation(f"{NUM_MULTI_GET}件取得", get_multi_with_validate, test_keys_multi)
    ...
    time_construct = time_operation(f"{NUM_MULTI_GET}件取得", get_multi_with_construct, test_keys_multi)

    # --- 結果サマリ ---
    logger.info("\n--- 複数取得 結果サマリ ---")
    logger.info(f"{NUM_MULTI_GET}件取得時間: model_validate = {time_validate:.6f} 秒, model_construct = {time_construct:.6f} 秒")

# --- メイン実行 ---
if __name__ == "__main__":
    logger.info(f"===== Pydantic V2 パフォーマンス比較テスト開始 (エントリ数: {NUM_ENTRIES}) =====")
    ...
    profile_output_file = PROFILE_OUTPUT_DIR / f"pydantic_v2_compare_{NUM_ENTRIES}_entries.prof"
    profiler.dump_stats(profile_output_file)
    logger.info(f"\nプロファイル結果:\n{s.getvalue()}")
    logger.info(f"詳細なプロファイルデータは '{profile_output_file}' に保存されました。")
